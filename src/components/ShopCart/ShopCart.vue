...
import { CLEAR_CART } from '../../store/mutayion-types'
export default {
  name: 'ShopCart',
  data() {
    return {
    ...
    listShow() {
      ...
      if (this.isShow) {
        this.$nextTick(() => {
          ...
          /*
            1、创建对象前：判断对象不存在才去创建
            2、创建对象后：保存创建的对象  进行判断
          */
          if (!this.bscroll) {
            //可能高度还没到那个高度，此时不需要形成滑动
            this.bscroll = new BScroll(this.$refs.foods, {
              click: true
            })
          } else {
            this.bscroll.refresh() //让滚动对象刷新：重新计算来决定要不要形成滑动
          }
        })
      }
      return this.isShow
